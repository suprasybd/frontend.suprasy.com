...
const AdminThemeImage: React.FC<{
  fieldIndex: number;
  form: UseFormReturn<z.infer<typeof adminThemeSchema>>;
}> = ({ fieldIndex, form }) => {
  const {
    fields: adminThemeImages,
    append: appendImage,
    remove: removeImage,
    move: moveImage,
  } = useFieldArray({
                ...
                <div className="rounded-sm relative  border-gray-300 border-2">
                  ...
                  <div className="flex w-full ">
                    <button
                      onClick={(e) => {
                        ...
                        if (index !== 0) {
                          ...
                          moveImage(index, adminThemeImages.length - 1);
                        }
                      }}
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        if (index !== adminThemeImages.length - 1) {
                          moveImage(index, index + 1);
                        } else {
                          moveImage(index, 0);
                        }
